...
export class HazardRateRunService {
    ...
    constructor(server: string, database: string, username: string, passowrd: string) {
        this.runDbService = new HazardRateRunDbService(server, database, username, passowrd);
    }

    async calculate(runId: string) {
        try {
            await this.runDbService.initializeConnectionPool();
            console.log(new Date() + ` Starting Hazard Rate Run: ${runId}`);
            console.log(new Date() + " Changing Hazard Rate Run Status to 'Calculating'");
            await this.runDbService.updateHazardRateRunStatus(runId, HazardRateRunStatus.CALCULATING);

            console.log(new Date() + " Loading Hazard Rate Run Data");
            const {
                dayScorebands,
                statusScorebands,
            ...
            console.log(new Date() + " Done Loading Hazard Rate Run Data");

            console.log(new Date() + " Deleting Old Hazard Rate Run Results");
            ...
            console.log(new Date() + " Generating Hazard Rate Run Scenarios.");
            ...
            console.log(new Date() + ` Processing ${scenarios.length} Hazard Rate Run Scenarios.`);
            await this.processScenarios(scenarios, runParameters, absorbingStates, ageAnalysis, runId);
            console.log(new Date() + " Successfully Processed All Scenarios");
            await this.runDbService.updateHazardRateRunStatus(runId, HazardRateRunStatus.CALCULATED);
            console.log(new Date() + " Hazard Rate Run Complete.");
    ...
    private async fetchHazardRateRunData(runId: string) {
        try {
            console.log(new Date() + " Fetching Hazard Rate Run");
            const hazardRateRun = await this.runDbService.fetchHazardRateRun(runId);

            console.log(new Date() + " Fetching Score Bands");
    ...
    private async processScenarios(
        ...
        for (const scenario of scenarios) {
            ...
            const scenarioAgeAnalysis = filteredAgeAnalysis;
            const scenarioInput = {
            ...
            console.log(new Date() + ` Saving Scenario ${counter} to the Database`);
            ...
            if (dbScenario.id) {
                ...
                for (const item of scenario.cohortMigrationMatrix)
                    await this.runDbService.insertCohortMigrationListItem(item, scenarioId);

                for (const item of scenario.hazardRateMigrationMatrix)
                    await this.runDbService.insertHazardMigrationListItem(item, scenarioId);
            }

            console.log(new Date() + ` Saving Scenario ${counter} Completed`);
            counter++;
        }
    }
